...
def calculate_category(db: Engine, name: str, include_unknown: bool = False,
    ...
    for runner in runners:
        ...
        for punch in punches:
            if punch.code == 1000:
                start = punch.time.replace(tzinfo=timezone.utc)
            elif punch.code == 1001:
                finish = punch.time.replace(tzinfo=timezone.utc)
            elif punch.code in loc_controls:
                tx += 1
                control = list(filter(lambda c: c.code == punch.code, controls))
                order.append((control[0].name, punch.time.replace(tzinfo=timezone.utc), "OK"))
            else:
        ...
        if not start:
            start = api.get_basic_info(db)["date_tzero"].replace(tzinfo=timezone.utc)

        if not finish:
            status = "DNF"
            time = 0
        else:
            time = round((finish - start).seconds)
